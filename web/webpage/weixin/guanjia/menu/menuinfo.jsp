 	...
 	$(document).ready(function(){ 
 	   ...
 	   if(templateId){
 		...
 		$("#type").change(function(){
 			...
 			}else if("scancode_push"==selectValue){
				$("#url").attr("disabled","disabled");
				$("#trurl").attr("style","display:none");
		...
		if("click" == typeVal){
			...
			$("#msgType").removeAttr("datatype");
			$("#templateId").removeAttr("datatype");
		}else if("scancode_push" == typeVal){
			$("#xxtr").hide();
			$("#trurl").hide();
			$("#templateTr").hide();
			$("#msgType").removeAttr("datatype");
			$("#templateId").removeAttr("datatype");
		}else if("media_id" == typeVal){
			$("#xxtr").show();
			$("#trurl").hide();
			$("#templateTr").show();
			//设置消息类型和验证
    ...
    <tr>
     ...
     <td colspan="3" class="value">
      <select name="type" id="type">
		  ...
		  <option value="media_id"  <c:if test="${type=='media_id'}">selected="selected"</c:if>>图文触发类</option>
      </select>
